...
%% Panel c
%% Comparing results for the ED model (adapted from Froehlich et al., 2017) between CMA-ES and current results from DC

% enzymatic degradation (from Froehlich, F. et al., 2016)
tc = 'ed';
k = [2.25 ...  % (mu m^3 * h)^-1
     4 ...     % h^-1
     100 ...   % mu m^3 / h
...
if(nargin > 0)
  if(~strcmp(varargin{1},'analyze'))
    ...
    [f, t] = costfcnPSSA_CMAES([k(:)' 0 0 1 0],tc,ts,dt,te,1); % initialize the cost function
    [f, t] = costfcnPSSA_TTestFano([k(:)' 0 0 1 0],tc,ts,dt,te,1,1000,1000); % initialize the cost function
    ...
    for k_p = 2 : nargin
      if(2 == exist(varargin{k_p}, 'file'))
        ...
        if(strcmp(inparams.strMethod,'cmaes'))
          ...
          f1 = costfcnPSSA_CMAES([k_i(:)' k(7) 0 0 1 0]);
          [f2, t] = costfcnPSSA_TTestFano([k_i(:)' k(7) 0 0 1 0]);
    ...
    if(~isempty(DC_avg))
      ...
      f1 = costfcnPSSA_CMAES([DC_avg_plain(:)' k(7) 0 0 1 0]);
      [f2, t] = costfcnPSSA_TTestFano([DC_avg_plain(:)' k(7) 0 0 1 0]);
      fprintf(hFileDC, '\n# Estimated cost: %f vs (%f, %f)\n', f1, f2, t);
      fprintf(hFileDC, '# Volume-weighted average\n');
      for k_pk=1:length(DC_avg)
        fprintf(hFileDC, '%f ', DC_avg(k_pk));
      end
      f1 = costfcnPSSA_CMAES([DC_avg(:)' k(7) 0 0 1 0]);
      [f2, t] = costfcnPSSA_TTestFano([DC_avg(:)' k(7) 0 0 1 0]);
      fprintf(hFileDC, '\n# Estimated cost: %f vs (%f, %f)\n', f1, f2, t);
    end
    if(~isempty(CMAES_avg))
      ...
      f1 = costfcnPSSA_CMAES([CMAES_avg(:)' k(7) 0 0 1 0]);
      [f2, t] = costfcnPSSA_TTestFano([CMAES_avg(:)' k(7) 0 0 1 0]);
      fprintf(hFileCMAES, '\n# Estimated cost: %f vs (%f, %f)\n', f1, f2, t);
    end
    fclose(hFileDC);
    fclose(hFileCMAES);
...
if(strcmp(inparams.strMethod,'cmaes'))
  ...
  oraclePSSA_CMAES([k(:)' 0 0 1 0],'costfcnPSSA_CMAES',kidx,tc,inparams.dTimeStart,inparams.dTimeStep,inparams.dTimeEnd,num_moments);
  %
  % CMA-ES loop
...
elseif(strcmp(inparams.strMethod,'dc'))
  ...
  [t,f] = oraclePSSA_CMA_DC([k(:)' 0 0 1 0],'costfcnPSSA_TTestFano',kidx,4 + floor(3 * log(length(kidx))),0.1,tc,inparams.dTimeStart,inparams.dTimeStep,inparams.dTimeEnd,num_moments,num_repetitions,bootstrap_size);

...
// To compile:
// mex ./mex/mexpssa_odes.cpp -I~/.local/include -L~/.local/lib64 -lboost_thread -lboost_chrono -lboost_system -lut -l:libpssa.a -l:libgsl.a -l:libgslcblas.a -v -DHAVE_CONFIG_H -g
...
% Rate functions
F_R = [k(2); k(3)/k(7)^2; k(4)/k(7); k(5)/k(7); k(6)/k(7)]

mexpssa_odes(tc,ones(k(1),1),k)

problem.solver = 'lsqnonlin';
problem.x0 = [10; 10];
problem.lb = zeros(N, 1);
problem.objective = @(x) mexpssa_odes(tc,x,k);
problem.options = optimoptions('lsqnonlin','FiniteDifferenceType','central','Algorithm','levenberg-marquardt');

  x_mean = lsqnonlin(problem)
*/
    ...
    const mxArray *prhs[])
{
    ...
    size_t szSpecies = mxGetNumberOfElements(prhs[1]);
    ...
    /* local variables */
    mxArray * mxarODEs, * mxarJ, * mxarStoichiometry;
    ...
    mxarODEs = mxCreateDoubleMatrix(model.getSpeciesCount(), 1, mxREAL);

    computeF(model, arPopulation, ardF.get());
    computeODEs(model, ardF.get(), mxGetPr(mxarODEs));

    plhs[0] = mxarODEs;
    if(nlhs > 1)
    {
      // compute the Jacobian
      mxarJ = mxCreateDoubleMatrix(szSpecies, szSpecies, mxREAL);
